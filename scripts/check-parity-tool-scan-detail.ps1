...
$repo = "parity-tool-scan-flow"
$language = "Python"
$category = "CLI tools"

foreach ($path in @("metadata/domain-review.json", "config/review-profile.json", "fixtures/domain_review.csv", "docs/field-notes.md", "docs/review-journal.md", "examples/parity-tool-scan-walkthrough.md")) {
...
if ($ship -ne 3) { throw "$repo ship lane count changed" }
...
if ($profile.highest_case.name -ne "edge") { throw "$repo profile highest case changed" }
