...
`parity-tool-scan-flow` is centered on cli tools work in Python. The technical target is: Package a Python local lab for scan analysis with node-edge fixtures, cycle and reachability reports, and documented operating limits.
...
The score is deterministic. The current threshold is 162. Risk has a penalty of 6, latency has a penalty of 4, and weight contributes a bonus of 5.
